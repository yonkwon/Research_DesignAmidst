...
//  static final int N_IN_UNIT = 15;
  ...
  static final double NUM_TRIPLET = (double) (N * (N - 1) * (N - 2)) / 6;
...
//  static final double[] C = {0, .25, .5, .75, 1};
  static final int LENGTH_C = C.length;

  static final double P_LEARNING = .25;

  static final int[] RESULT_KEY_VALUE = {LENGTH_C, GRANULARITY_THETA, LENGTH_E, TIME};
  ...
  static String FILENAME = RUN_ID + (EXPERIMENT_IS_DECOMPOSITION ?
      //Decomposed
      "Dec_I" + ITERATION + "T" + TIME + "N" + N_OF_UNIT + "X" + N_IN_UNIT + "M" + M_OF_BUNDLE + "X" + M_IN_BUNDLE + "C" + LENGTH_C + "Be" + GRANULARITY_THETA + "(Opt" + "" + (GET_OPTIMAL_BETA ? "Calc"
          : "Set") + ")" + "E" + LENGTH_E + "P" + P_LEARNING :
      //Not Decomposed
      "Com_I" + ITERATION + "T" + TIME + "N" + N_OF_UNIT + "X" + N_IN_UNIT + "M" + M_OF_BUNDLE + "X" + M_IN_BUNDLE + "C" + LENGTH_C + "Et" + LENGTH_BETA + "E" + LENGTH_E + "P" + P_LEARNING);
  static String PATH_CSV = new File(".").getAbsolutePath() + "\\" + FILENAME + "\\";

  public static void main(String[] args) {
    System.out.println("Closure: Baseline");
