...
public class Scenario {
  ...
  private void initializeNetwork() {
    ...
    while (currentDepthIndex < Main.N) {
      int depthStart = currentDepthIndex;
      // For all nodes at the current depth, assign subordinates
      for (int i = depthStart; i < depthStart + FastMath.pow(span, (currentDepthIndex / span)) && i < Main.N; i++) {
        // Don't exceed population
        if (nextNodeId + span <= Main.N) {
          for (int j = 0; j < span; j++) {
            network[i][nextNodeId] = true;
            network[nextNodeId][i] = true;
            nextNodeId++;
          }
        } else {
          // Assign as many as possible
          while (nextNodeId < Main.N) {
            network[i][nextNodeId] = true;
            network[nextNodeId][i] = true;
            nextNodeId++;
          }
      ...
      // Advance to the next depth level in the hierarchy
      currentDepthIndex += (int) FastMath.pow(span, (currentDepthIndex / span));
    }
