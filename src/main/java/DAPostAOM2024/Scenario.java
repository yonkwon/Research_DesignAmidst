...
public class Scenario {
  ...
  boolean isHomophilyOnKnow;
  boolean isHeterophiilyOnKnow;
  ...
  Scenario(int socialMechanism, int span, double connectivity, double enforcement) {
    r = new MersenneTwister();

    this.socialMechanism = socialMechanism;
    switch (socialMechanism) {
      case 0 -> isHomophilyOnChar = true;
      case 1 -> isHomophilyOnKnow = true;
      case 2 -> isHeterophiilyOnKnow = true;
      case 3 -> isNetworkClosure = true;
      case 4 -> isPreferentialAttachment = true;
    }
  ...
  public Scenario getClone() {
    Scenario clone = new Scenario(this.socialMechanism, this.span, this.enforcement, this.connectivity);
...
//    return r.nextDouble();
  ...
  double getNeighborScoreHomophilyOnKnowledge(int focal, int target) {
    return 1D - FastMath.abs(performance[focal] - performance[target]) / (performanceRange != 0 ? performanceRange : 1);
//    return r.nextDouble();
  }

  double getNeighborScoreHeterophilyOnKnowledge(int focal, int target) {
    double neighborScore = 0;
    for (int m = 0; m < Main.M; m++) {
      if (beliefOf[focal][m] != beliefOf[target][m]) {
        neighborScore++;
    ...
    return neighborScore / (double) Main.M;
//    return r.nextDouble();
  }
...
//    return r.nextDouble();
  ...
  void doEvaluateNeighbor() {
    ...
    } else if (isHomophilyOnKnow) {
      for (int focal : focalIndexArray) {
        for (int target = focal; target < Main.N; target++) {
          if (!network[focal][target]) {
            continue;
          }
          neighborScore[focal][target] = getNeighborScoreHomophilyOnKnowledge(focal, target);
          neighborScore[target][focal] = neighborScore[focal][target];
          neighborhoodScore[focal] += neighborScore[focal][target];
          neighborhoodScore[target] += neighborScore[target][focal];
        }
      }
    } else if (isHeterophiilyOnKnow) {
      for (int focal : focalIndexArray) {
        for (int target = focal; target < Main.N; target++) {
          if (!network[focal][target]) {
            continue;
          }
          neighborScore[focal][target] = getNeighborScoreHeterophilyOnKnowledge(focal, target);
          neighborScore[target][focal] = neighborScore[focal][target];
          neighborhoodScore[focal] += neighborScore[focal][target];
          neighborhoodScore[target] += neighborScore[target][focal];
        }
        ...
        for (int target : targetIndexArray) {
          if (
            ...
            if (isHomophilyOnChar) {
              ...
              targetScore = getNeighborScoreHomophilyOnKnowledge(focal, target);
            } else if (isHeterophiilyOnKnow) {
              targetScore = getNeighborScoreHeterophilyOnKnowledge(focal, target);
            } else if (isNetworkClosure) {
