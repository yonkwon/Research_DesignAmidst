...
class MatWriter {
  ...
  MatWriter(Computation d) {
    ...
    Matrix matrixArraySpan = Mat5.newMatrix(new int[]{1, Main.LENGTH_SPREAD});
    IntStream.range(0, Main.LENGTH_SPREAD).forEach(i -> matrixArraySpan.setDouble(new int[]{0, i}, Main.SPREAD[i]));
    ...
    try {
      Mat5.newMatFile()
          ...
          .addArray("para_g_span", Mat5.newScalar(Main.LENGTH_SPREAD))
